...
class UsersTab(QWidget):
    ...
    def __init__(self, api, user_data, parent=None): 
        ...
        # Main layout with proper scroll area
        ...
        # Add the buttons to the search card
        ...
        # Add button layouts to search card
        search_layout.addLayout(selection_layout)
        ...
        """)
        self.list_users.setMinimumHeight(200)
        
        # Progress bar (hidden by default but still created for functionality)
        self.user_bar = QProgressBar()
        self.user_bar.setMaximumHeight(10)
        self.user_bar.setTextVisible(False)
        self.user_bar.setVisible(False)  # Hide the progress bar
        
        # Create the log but make it invisible (needed for functionality)
        self.user_log = QTextEdit()
        self.user_log.setVisible(False)
        
        # Only add the list to results layout
        results_layout.addWidget(self.list_users)
        
        # Add results card to container
        container_layout.addWidget(results_card)
    ...
    def multi_follow(self):
        ...
        # Get tokens directly from the parent window
        all_tokens = {}
        if hasattr(self.parent_window, 'all_tokens'):
            all_tokens = self.parent_window.all_tokens
...
class StarsTab(QWidget):
    ...
    def __init__(self, api, user_data, parent=None):  # Add parent parameter
        super().__init__(parent)  # Pass parent to super constructor
        self.api = api
        self.user_data = user_data
        ...
        # Progress bar (hidden but created for functionality)
        self.repo_bar = QProgressBar()
        self.repo_bar.setMaximumHeight(10)
        self.repo_bar.setTextVisible(False)
        self.repo_bar.setVisible(False)
        
        # Log area (hidden but created for functionality)
        self.repo_log = QTextEdit()
        self.repo_log.setVisible(False)
        
        container_layout.addWidget(repo_card)
